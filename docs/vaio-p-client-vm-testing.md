...
С релизной страницы GitHub скачать архив для i686:

```sh
wget https://github.com/akberdin/ph0sphor/releases/download/vX.Y.Z/ph0sphor-vX.Y.Z-linux-i686.tar.gz
wget https://github.com/akberdin/ph0sphor/releases/download/vX.Y.Z/ph0sphor-vX.Y.Z-linux-i686.tar.gz.sha256
sha256sum -c ph0sphor-vX.Y.Z-linux-i686.tar.gz.sha256
tar xzf ph0sphor-vX.Y.Z-linux-i686.tar.gz
cd ph0sphor-vX.Y.Z-linux-i686
```

Бинарь статический (musl), доп. библиотеки antiX ставить не нужно.

Если в релизах нет нужной версии — соберите из исходников на хосте:
...
И скопируйте `target/i686-unknown-linux-musl/release/ph0sphor-client`
в ВМ (через общую папку VirtualBox, `scp` или USB-флешку).
